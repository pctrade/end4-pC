...
Item {  
                ...
                StyledTextArea {  
                    ...
                    onTextChanged: {
                        ...
                        if (text.startsWith(`${prefix}service`)) {
                            root.suggestionQuery = text.split(" ").slice(1).join(" ")
                            root.suggestionList = [
                                { name: `${prefix}service unsplash`, description: Translation.tr("Use Unsplash (requires API key)") },
                                { name: `${prefix}service wallhaven`, description: Translation.tr("Use Wallhaven (no API key required)") }
                            ]
                        } else if (text.startsWith(`${prefix}sort`)) {
                            root.suggestionQuery = text.split(" ").slice(1).join(" ")
                            root.suggestionList = (root.sortOptions[root.currentService] ?? []).map(opt => ({
                                name: `${prefix}sort ${opt}`, description: ""
                            }))
                        } else if (text.startsWith(`${prefix}api`)) {
                            root.suggestionQuery = text.split(" ").slice(1).join(" ")
                            root.suggestionList = root.currentService === "wallhaven"
                                ? [{ name: `${prefix}api`, description: Translation.tr("Wallhaven doesn't require an API key") }]
                                : [{ name: `${prefix}api YOUR_KEY`, description: Translation.tr("Set your Unsplash API key") }]
                        } else if (text.startsWith(`${prefix}anime`)) {
                            root.suggestionList = [
                                { name: `${prefix}anime show`, description: Translation.tr("Shows anime-related results") },
                                { name: `${prefix}anime hide`, description: Translation.tr("Hides anime-related results") }
                            ]
                    ...
                    Keys.onPressed: event => {  
                        if (event.key === Qt.Key_Return && event.modifiers & Qt.ShiftModifier) {
                            ...
                            if (root.suggestionList.length > 0) {
                                searchInputField.text = root.suggestionList[suggestions.selectedIndex].name + " "
                                searchInputField.cursorPosition = searchInputField.text.length
                                searchInputField.forceActiveFocus()
                            }
                            event.accepted = true
                        } else if (event.key === Qt.Key_Up) {
                            if (suggestions.selectedIndex > 0) suggestions.selectedIndex--
                            event.accepted = true
                        } else if (event.key === Qt.Key_Down) {
                            if (suggestions.selectedIndex < root.suggestionList.length - 1) suggestions.selectedIndex++
                            event.accepted = true
                        }
                    }
                }  
    ...
    property var allCommands: [  
        { name: "api", description: Translation.tr("Set API key for current service. Usage: %1api YOUR_API_KEY").arg(root.commandPrefix), execute: args => {
            ...
            if (args.length === 0) {
                if (WallpaperBrowser.unsplashApiToken !== "") {
            ...
            if (args[0].length < 20) {
                ...
                KeyringStorage.setNestedField(["apiKeys", `wallpapers_${root.currentService}`], "")
                return
            }
            KeyringStorage.setNestedField(["apiKeys", `wallpapers_${root.currentService}`], args[0].trim())
            WallpaperBrowser.addSystemMessage(Translation.tr("API key set for %1").arg(root.currentService))
        }},
        ...
        { name: "anime", description: Translation.tr("Toggle anime results. Usage: %1anime SHOW/HIDE").arg(root.commandPrefix), execute: args => {
            if (!requireWallhaven(Translation.tr("Anime toggle"))) return
            if (args.length === 0) {
            ...
            WallpaperBrowser.addSystemMessage(Translation.tr("Anime results: %1").arg(show ? "visible" : "hidden"))
            WallpaperBrowser.setAnimeResults(show)
        }},
        { name: "sort", description: Translation.tr("Sort results. Usage: %1sort SORT_OPTION").arg(root.commandPrefix), execute: args => {
            const validSorts = root.sortOptions[root.currentService] ?? []
            if (args.length === 0) {
                WallpaperBrowser.addSystemMessage(Translation.tr("Please add a sort option\nAvailable sorts: %1").arg(validSorts.join(", ")))
                return
            }
            ...
            if (validSorts.includes(sort)) {
                ...
                WallpaperBrowser.addSystemMessage(Translation.tr("Invalid sort option. Use: %1").arg(validSorts.join(", ")))
            }
        }},
        ...
        { name: "next", description: Translation.tr("Load next page"), execute: () => {
            ...
            if (lastResponse.page <= 0) return
            if (WallpaperBrowser.similarImageId !== "") {
    ...
    function handleInput(inputText) {
        ...
        if (inputText.startsWith(root.commandPrefix)) {
            ...
            if (tags.length > 0) WallpaperBrowser.makeRequest(tags, root.imageLimit, page)
        }
    }
}